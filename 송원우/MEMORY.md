...
| 2026-05-12 | **튜터 피드백 5종 반영**. **(①) brand_page.py 워드클라우드 하단 3패널 막대 추가**: `_render_keyword_trio()` 신규 — 별점 4–5점 긍정 Top10 / 1–2점 부정 Top10 / cat1 Top1 카테고리 Top10. `_EXTRA_STOPWORDS` 재사용. ABSA 대비 표본 풀 활용도 大 (모든 리뷰). **(②) 2_상품_및_고객_전략 Action Recommendation 정제**: `**...**` 마크다운 → `<strong>...</strong>` HTML 태그. 절 단위 `<br/>` 삽입, 폰트 14px / line-height 1.7로 통일. **(③-A) 5_전략_포지셔닝 좌표 산식 토글**: `positioning_map.py`에 `x_range`/`y_range`/`x_title`/`y_title` 인자 추가, `_add_quadrants(x_range, y_range)`로 사분면 중심·라벨 동적 계산. 페이지 상단에 산식 라디오 ['A. 현재 (P−N)/(P+N)', 'C. 발화량 × 강도 (1−X)·(P−N)'] + '축 확대 (산식 A 전용)' 토글. 산식 C: `_compute_pos_c()` 신규 — `(1-X_ratio)*(P_ratio-N_ratio)`. 실측 좌표 — FILA (0.179, 0.158), 안다르 (0.413, 0.232), 젝시믹스 (0.360, 0.108), 룰루레몬 (0.491, 0.262). gap 0.054→0.312로 6배 확대. 축 범위 자동 조정(C: 0~0.6×0~0.35 / A: 0.88~1.00 또는 0~1). 시뮬레이터 슬라이더도 산식별 범위·기본값 자동 전환. expander에 두 산식 비교표 게재. **(③-B) PMI 시드 확장 + 신발:의류 5.6:1 인사이트 박스**: `_APPAREL_SEEDS` 16개 → 55개(티/후드/팬츠/조거/요가복/카고 등 추가). PMI 섹션 상단에 노란 인사이트 박스 추가 — "FILA 토큰 풀 신발 10,946건 vs 의류 1,946건 = 5.6:1. PMI 그래프에 의류 노드 적은 것은 모델 문제 아닌 데이터 시그널. 신발→의류 인식 전환이 White Space의 본질". **(④) White Space `**` 정제 + Option A 추천 강조**: 카드 본문 `**...**` → `<strong>...</strong>` + `<br/>`. Option A 카드만 border 4px(다른 카드 2px) + glow + ⭐권장 뱃지. 카드 위에 보라색 그라데이션 추천 배너 추가. Phase 2 로드맵 항목에 ⭐ 표시. **검증**: 5페이지 AppTest exc=0/err=0. 산식 A·C 토글 모두 0건. PDF 952KB 재출력. |
